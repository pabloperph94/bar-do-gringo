...
function openCustomizeModal(item) {
  ...
  content.querySelectorAll('.adicional-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (action === 'plus') {
        selectedAdicionais[adId] = (selectedAdicionais[adId] || 0) + 1;
      } else {
        selectedAdicionais[adId] = Math.max(0, (selectedAdicionais[adId] || 0) - 1);
      }
...
document.getElementById('order-form').addEventListener('submit', async function (e) {
  ...
  try {
    ...
    if (data.success) {
      ...
      const linkWpp = document.createElement('a');
      linkWpp.href = `https://wa.me/${whatsappNumero}?text=${encodeURIComponent(msg)}`;
      linkWpp.target = '_blank';
      linkWpp.rel = 'noopener noreferrer';
      document.body.appendChild(linkWpp);
      linkWpp.click();
      document.body.removeChild(linkWpp);
    } else {
